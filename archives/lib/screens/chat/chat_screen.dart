...
class _ChatScreenState extends ConsumerState<ChatScreen> {
  ...
  void _jumpToMessage(
    ...
    if (targetIndex < 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Could not highlight that message')),
      );
      return;
    }
  ...
  void _toggleMessageSelection(MessageModel message, bool isMe) {
    if (!isMe) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('You can only select your own messages')),
      );
      return;
    }
  ...
  Future<void> _deleteSelectedMessages() async {
    ...
    try {
      ...
      if (mounted) {
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('Deleted $deletedCount message(s)')),
        );
      }
    } catch (e) {
      if (mounted) {
